...
class Ros0xRobotNode
{
    ...
    bool enableSonar;
    bool enableImu;
    bool enableodomtf;//for publishing odometry transform
    ...
    n(nh)
{
    ...
    n.param( "enableodomtf", enableodomtf, true);
    ROS_INFO( "Ros0xRobotNode: Odomtf enabled: %d", enableodomtf );
    ...
    frame_id_base_link = tf::resolve(tf_prefix, "base_footprint");
...
void Ros0xRobotNode::getPosition(nav_msgs::Odometry *position)
{
    ...
    if(deltaX>1000||deltaX<-1000)deltaX=0;//discard dirty data beyond maximum speed
    if(deltaY>1000||deltaY<-1000)deltaY=0;//discard dirty data beyond maximum speed
    if(deltaTheta>(M_PI/10)||deltaTheta<-(M_PI/10))deltaTheta=0;//discard dirty data beyond maximum speed
    x += deltaX;
    y += deltaY;
    theta += deltaTheta;
